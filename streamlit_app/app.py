...
def load_fine_tuned_embeddings():
    embeddings = np.load(os.path.join(streamlit_app_data_path,'fine_tuned_embeddings.npy'))
    return embeddings

@st.cache_resource
def load_default_embeddings():
    embeddings = np.load(os.path.join(streamlit_app_data_path, 'default_embeddings.npy'))
    return embeddings
